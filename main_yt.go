...
const YtHelpMsg = `Usage: %s [-][ytjcrneikhv]

Convert between YAML, TOML, JSON, and HCL.
Preserves map order.

-x[x]  Convert using stdin. Valid options:
          -yt, -y = YAML to TOML (default)
          -yj     = YAML to JSON
          -yy     = YAML to YAML
          -yc     = YAML to HCL
          -tj, -t = TOML to JSON
...
func main() {
	os.Exit(RunYt(os.Stdin, os.Stdout, os.Stderr, os.Args))
}

func RunYt(stdin io.Reader, stdout, stderr io.Writer, osArgs []string) (code int) {
	config, err := parseWithDefault(osArgs[1:], "yt")
	if err != nil {
		fmt.Fprintf(stderr, YtHelpMsg, os.Args[0])
		fmt.Fprintf(stderr, "Error: %s\n", err)
		return 1
	}
	if config.Help {
		fmt.Fprintf(stdout, YtHelpMsg, os.Args[0])
		return 0
	}
	if config.Version {
